 ...
 * @author Rodrigo Coelho <rodrigo@giix.org>
 */
abstract class tempimporter1GxActiveRecord extends CActiveRecord1 {
	...
	public static function label($n = 1) {
		throw new Exception(YiiBase1::t('giix', 'This method should be overriden by the Active Record class.'));
	}
		...
		foreach ($relNames as $relName) {
			...
			if (!isset($relations[$relName])) {
				...
				if ($relIndex === $countRelNames) {
					...
					throw new InvalidArgumentException(YiiBase1::t('giix', 'The attribute "{attribute}" should be the last name.', array('{attribute}' => $relName)));
				}
			}
	...
	public function fillPkColumnNames($pk) {
		...
		if (count($pk) !== $columnCount)
			throw new InvalidArgumentException(YiiBase1::t('giix', 'The count of values in the argument "pk" ({countPk}) does not match the count of columns in the composite PK ({countColumns}).'), array(
				'{countPk}' => count($pk),
	...
	protected function saveRelated($relatedData, $runValidation = true, $batch = true) {
		...
		if ($this->getIsNewRecord())
			throw new CDbException(YiiBase1::t('giix', 'Cannot save the related records to the database because the main record is new.'));
		...
		foreach ($relatedData as $relationName => $relationData) {
			...
			if (is_array($thisPkValue))
				throw new Exception(YiiBase1::t('giix', 'Composite primary keys are not supported.'));
							...
							if ($relationData[0] === GxActiveRecord::BELONGS_TO) {
								...
								if ($model->$fkName === null) {
									...
									if (isset($savedModels[$relatedClassName])) {
										...
										throw new Exception(YiiBase1::t('giix', 'Related model not found. Cannot continue without filling up the FK.'));
									}
								}
							}
						}
