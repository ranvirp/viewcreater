...
//extends CBaseController
{
	...
	{
		throw new CHttpException(400,YiiBase1::t('yii','Your request is invalid.'));
	}
	...
	{
		YiiBase1::app()->getClientScript()->render($output);
	...
	public function createAction($actionID)
	{
		...
		else
		{
			...
			if($action!==null && !method_exists($action,'run'))
				throw new CException(YiiBase1::t('yii', 'Action class {class} must implement the "run" method.', array('{class}'=>get_class($action))));
			return $action;
		}
	}
	...
	protected function createActionFromMap($actionMap,$actionID,$requestActionID,$config=array())
	{
		if(($pos=strpos($actionID,'.'))===false && isset($actionMap[$actionID]))
		{
			...
			return YiiBase1::createComponent(empty($config)?$baseConfig:array_merge($baseConfig,$config),$this,$requestActionID);
		}
		elseif($pos===false)
			return null;
		...
		elseif(is_array($provider) && isset($provider['class']))
		{
			...
			throw new CException(YiiBase1::t('yii','Object configuration must be an array containing a "class" element.'));

		$class=YiiBase1::import($providerType,true);
	...
	{
		throw new CHttpException(404,YiiBase1::t('yii','The system is unable to find the requested action "{action}".',
	...
	{
		if(($theme=YiiBase1::app()->getTheme())!==null && ($viewFile=$theme->getViewFile($this,$viewName))!==false)
			return $viewFile;
	...
	public function getLayoutFile($layoutName)
	{
		if($layoutName===false)
			return false;
		if(($theme=YiiBase1::app()->getTheme())!==null && ($layoutFile=$theme->getLayoutFile($this,$layoutName))!==false)
			return $layoutFile;
		...
		return $this->resolveViewFile($layoutName,$module->getLayoutPath(),YiiBase1::app()->getViewPath(),$module->getViewPath());
	}
...
public function resolveViewFile1($viewName,$viewPath,$basePath,$moduleViewPath=null)
	...
	public function forward($route,$exit=true)
	{
		...
		else
		{
			...
			YiiBase1::app()->runController($route);
		}
		if($exit)
			YiiBase1::app()->end();
	}
	...
	public function renderPartial($view,$data=null,$return=false,$processOutput=false)
	{
		if(($viewFile=$this->getViewFile($view))!==false)
		{
			...
			throw new CException(YiiBase1::t('yii','{controller} cannot find the requested view "{view}".',
	...
	public function createUrl($route,$params=array(),$ampersand='&')
	{
		...
		if($route[0]!=='/' && ($module=$this->getModule())!==null)
			$route=$module->getId().'/'.$route;
		return YiiBase1::app()->createUrl(trim($route,'/'),$params,$ampersand);
	}
	...
	public function createAbsoluteUrl($route,$params=array(),$schema='',$ampersand='&')
	{
		...
		if(strpos($url,'http')===0)
			...
			return YiiBase1::app()->getRequest()->getHostInfo($schema).$url;
	}
	...
	public function getPageTitle()
	{
		...
		else
		{
			...
			if($this->getAction()!==null && strcasecmp($this->getAction()->getId(),$this->defaultAction))
				return $this->_pageTitle=YiiBase1::app()->name.' - '.ucfirst($this->getAction()->getId()).' '.$name;
			else
				return $this->_pageTitle=YiiBase1::app()->name.' - '.$name;
		}
	}
	...
	public function redirect($url,$terminate=true,$statusCode=302)
	{
		...
		YiiBase1::app()->getRequest()->redirect($url,$terminate,$statusCode);
	}
	...
	{
		$this->redirect(YiiBase1::app()->getRequest()->getUrl().$anchor,$terminate);
	}
	...
	{
		if(YiiBase1::app()->getRequest()->getIsPostRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,YiiBase1::t('yii','Your request is invalid.'));
	}
	...
	{
		if(YiiBase1::app()->getRequest()->getIsAjaxRequest())
			$filterChain->run();
		else
			throw new CHttpException(400,YiiBase1::t('yii','Your request is invalid.'));
	}
	...
	protected function loadPageStates()
	{
		if(!empty($_POST[self::STATE_INPUT_NAME]))
		{
			if(($data=base64_decode($_POST[self::STATE_INPUT_NAME]))!==false)
			{
				...
				if(($data=YiiBase1::app()->getSecurityManager()->validateData($data))!==false)
	...
	{
		$data=YiiBase1::app()->getSecurityManager()->hashData(serialize($states));
		if(extension_loaded('zlib'))
