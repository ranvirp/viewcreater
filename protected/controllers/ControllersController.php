...
class ControllersController extends Controller
{
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function actionHtmlentities()
        {
            $src=$_GET['src'];
            print htmlentities($src);
        }
        public function actionWriteC1()
        {
            $controller=$_GET['controller'];
            $model= Controllers::model()->findByPk($controller);
            $name= $model->name;
            $path= $model->path;
            $functions=Functions::model()->findAllByAttributes(array('controller'=>$controller));
            $str="<?php\n";
            $str.=$model->comments;
            ...
            $str.="\n}\n";
            file_put_contents($path.'/'.$name.'Controller.php',$str);
        }
